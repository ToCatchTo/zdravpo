	...
	return (
		<Box id='header'>
			...
			<Grid component="header" columns={14} size={14} container spacing={2} sx={{ display: { sm: 'flex', xs: 'none' } }}>
				...
				{/* Rozšířená navigace */}
				<Grid size={13} spacing={2} container columns={13} sx={{ backgroundColor: colors.primary, color: colors.text, height: '100vh', borderRadius: { lg: '0px 0px 300px 0px', md: '0px 0px 200px 0px', sm: '0px 0px 100px 0px' }, ml: '-16px', zIndex: 998, position: 'absolute', transform: isOpened ? 'translateX(0)' : 'translateX(-50vw)', transition: 'transform 0.5s ease' }}>
					<Grid size={{ md: 3, sm: 4 }} offset={7} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%' }}>
						<Box sx={{ display: 'flex', flexDirection: 'column', gap: { md: '40px', sm: '10px' }, '@media (min-width: 1536px)': { gap: '65px' }, '@media (min-width: 1700px)': { gap: '100px' } }}>
							...
							<Box sx={{ pb: { lg: '144px', md: '124px', sm: '84px' }, '@media (min-width: 1536px)': { pb: '164px' }, '@media (min-width: 1700px)': { pb: '184px' } }}>
								...
								<Box component="a" onClick={() => navigate("/payMethods")} sx={{ textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', pr: { md: '15px', sm: '5px' }, height: { lg: '70px', md: '60px', sm: '40px' }, '@media (min-width: 1536px)': { height: '80px' }, '&:hover': { textDecoration: 'underline' }, color: colors.secondary, '@media (min-width: 1700px)': { height: '90px' } }}>
				...
				<Box sx={{ position: 'fixed', width: '100vw', height: '100vh', backgroundColor: colors.primary, zIndex: 999, top: 0, left: 0, display: 'flex', alignItems: 'center', flexDirection: 'column', overflow: 'auto', transform: isOpened ? 'translateY(0)' : 'translateY(-100vh)', transition: 'transform 0.5s ease' }}>
					...
					<Box sx={{ backgroundColor: colors.primary, width: '280px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pt: '69px' }}>
						{mobileMenuItems.map((item, i) => (
							<Box component="a" href='#' key={i} sx={{ textDecoration: 'none', display: 'flex', alignItems: 'center', cursor: 'pointer', borderBottom: i === mobileMenuItems.length - 1 ? 'none' : `1px solid ${colors.secondary}`, borderTop: i === 0 ? `1px solid ${colors.secondary}` : 'none', height: '65px', width: '100%', mb: i === mobileMenuItems.length - 1 ? '60px' : '0px', '&:hover': { textDecoration: 'underline' }, color: colors.secondary }}>
